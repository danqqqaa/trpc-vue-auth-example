...
import * as placesJson from './places.json';
...
export class PlaceService implements OnModuleInit {
  constructor(
    ...
    @InjectModel(TransportTypePlaceAssociation, 'GAZELLE_REPOSITORY')
    private readonly transportTypePlaceAssociationRepository: typeof TransportTypePlaceAssociation,
  ...
  async onModuleInit() {
    if (this.configService.get<string>('RESET_DB') === 'true') {
      await this.placeRepository.bulkCreate(
        placesJson.map((p) => ({ name: p })),
      );
    }
  }
  ...
  async findOne(id: number) {
    ...
    if (!place) throw new NotFoundException('Place not founded!');
    return place;
  }

  async create(createPlaceDto: CreatePlaceDto) {
    const place = await this.placeRepository.create({ ...createPlaceDto, addedManualy: true });
    for (let i = 0; i < createPlaceDto.transportTypes?.length; i++) {
      await this.transportTypePlaceAssociationRepository.create({
        placeId: place.id,
        transportTypeId: createPlaceDto.transportTypes[i].id
      })
    }
    AppGateway.instance.placeCreate(place);
    return place;
  }
  ...
  async update(id: number, updatePlaceDto: UpdatePlaceDto) {
    ...
    await this.transportTypePlaceAssociationRepository.destroy({
      where: {
        placeId: place.id,
      },
    });
    for (let i = 0; i < updatePlaceDto.transportTypes?.length; i++) {
      await this.transportTypePlaceAssociationRepository.create({
        placeId: place.id,
        transportTypeId: updatePlaceDto.transportTypes[i].id
      })
    }
  ...
  async findOneOrCreateByName(name: string) {
    ...
    let place
    place = places.find((element) => element.addedManualy === true)
    if (place) return place;
    place = places.find((element) => element.addedManualy === false)
    if (place) return place;
    const newPlace = await this.placeRepository.create({ name });
    AppGateway.instance.placeCreate(newPlace);
    return newPlace;
  }
}
